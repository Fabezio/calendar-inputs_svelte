...
{#each jobs as job}
	{#if job.date.split('-')[1] === '09'}
		<div
			class={job.isFerie ||
			formatter.format(new Date(job.date)).split(' ')[0] == 'samedi' ||
			formatter.format(new Date(job.date)).split(' ')[0] == 'dimanche'
				? 'text-success'
				: ''}
		>
			{formatter.format(new Date(job.date))} :
			<span> {job.chefJour.nom}, {job.chefNuit.nom} </span>;
			<span> {job.agentJour.nom}, {job.agentNuit.nom} </span>
		</div>
	{/if}
	<!-- <div>{job.date}</div> -->
{/each}
